...
$sub = 0;
/*	
$sqlid= "select * from db_usuarios where login = '$perfil'";
$resultid = pg_exec($sqlid);
$li = pg_num_rows($resultid);
if($li>0){
	$usuario=pg_result($resultid, 0,"id_usuario");
}*/
//echo "id= $usuario";
  // entra aqui
	...
	$rotinadb = "";
	// entra aqui
	if ($NumMenu != 0) { //position:absolute;left:0px;top:0px... tirei para não ficar la em cima
						...
						if (pg_result($menu, $j, "id_item") == pg_result($menu, $a, "id_item")) { 
							...
							$funcao2 = trim(pg_result($menu, $a, 'funcao'));
								if($sub==1){
									...
									//echo "<a class=\"menuItem\" target=\"CentroPref\" id=\"DBmenu_".pg_result($menu, $a, "id_item_filho")."\" href=\"".pg_result($menu, $a, "funcao")."\"  onclick='return js_db_menu_confirma();'>".ucfirst(pg_result($menu, $a, "descricao"))."</a>\n";
								}
							}
						}
					}
	 ...
	 // entra aqui...
		  ...
		  $resultmodulo = pg_exec($sqlmodulo) or die($sqlmodulo);
