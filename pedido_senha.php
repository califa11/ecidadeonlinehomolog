...
if (isset($cgccpf)) {
  ...
  $result = @pg_query($sql) or die(@pg_errormessage());
  ...
  if(@pg_num_rows($result) == 0){
   ...
   $result1 = @pg_query("select login from db_usuarios where login = '$z01_numcgm'");
   $linhas1 = @pg_num_rows($result1);
	 ...
	 if ($linhas1==0) {
		 ...
		 $result = pg_query($sqlusu);
		 $id_usuario = pg_result($result,0,0);

		 
		 $result2 = @pg_query("insert into db_usuarios ( id_usuario,
		                                                 nome,
		                  login,
		                  senha,
		                  usuarioativo,
		                  ...
		                  '1',
		                  '$z01_email',
		                  1)") or die("Erro(23) inserindo em db_usuarios: ".pg_errormessage());
		    $result3 = @pg_query("insert into db_usuacgm (id_usuario,cgmlogin) values($id_usuario,$z01_numcgm)");
    		...
    		$mensagemDestinatario = "
  		    ...
  		    $result3 = @pg_query("update db_usuarios set senha = '$passwd2' where login = '$dados[0]'");
...
//      echo $mensagemDestinatario;
      $oMail    = new mail();
      $oMail->setsEmailTo($z01_email);
      $oMail->setsSubject("Pedido de Senha Internet - Prefeitura On-Line");
      $oMail->setsMsg(nl2br($mensagemDestinatario));
      $sMsgMail = $oMail->Send();
      db_msgbox($sMsgMail);
...
<table width="80%" align="center" border="0" cellpadding="1" cellspacing="1" bgcolor="<?$w01_corbody?>" class="bold10">
  ...
  <tr>
    ...
    <?  
     $sSeleciona = array ("2" => "Servidor Municipal",
 			  "1" => "Contribuinte"
                         );
...
str = "<span><font color='#E9000'> SUA SENHA DEVE CONTER NO MÍNIMO 6 CARACTERES, LETRAS E NÚMEROS! </font></span>"; 
...
// varametro para verificar caracteres digitados   
if(sT == 'S'){   
...
<script>js_redireciona(2)</script>
