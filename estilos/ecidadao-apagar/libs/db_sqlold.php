...
$sql = "select   y.k00_inscr,
  ...
  $sql .= ") as x
   ...
   ";
  $result = pg_exec($sql) or die("Sql : ".pg_ErrorMessage($result));
  if($limite == 0 ) {
...
function db_mensagem($cabecalho,$rodape) {
  ...
  $result = pg_exec("select mens,alinhamento from db_confmensagem where cod = '$cabecalho' or cod = '$rodape' order by cod");
  if (pg_numrows($result) == 0 ){
...
function debitos_iptu($matricula,$limite,$anoexe){
  ...
  if ( $anoexe != 0 ) {
     ...
     $result_u = pg_exec($sqlu) or die("Sql : ".pg_ErrorMessage($result_u));	  
