...
include_once ("classes/db_db_bancos_classe.php");
include_once ("classes/db_iptubase_classe.php");
include_once ("classes/db_modcarne_classe.php");

include_once ("model/regraEmissao.model.php");
include_once ("model/convenio.model.php");
...
if ($k03_tipo == 1 && $impmodelo <> 1 && $impmodelo <> 30) {
	...
	try {
		$oRegraEmissao = new regraEmissao($tipo_debito, 1, db_getsession('DB_instit'), date("Y-m-d", db_getsession("DB_datausu")), db_getsession('DB_ip'));
  ...
  and k00_dtvenc < '" .date("Y-m-d", db_getsession("DB_datausu"))."' limit 1";
	...
	$resultunica = pg_exec("select * from recibounica where k00_numpre = $codigos[0] $whereDatasUnicas ");
	...
	if ($unica == 't') {
		...
		if ($resultfin != false) {
			...
			$numpre = db_numpre($k00_numpre).'000';
			//db_formatar(0,'s',3,'e');
		...
		if ($unica == 1 && $datasUnicas != "") {
			...
			if ($resultfin != false) {
				for ($unicont = 0; $unicont < pg_numrows($resultfin); $unicont++) {
					...
					$pdf1->titulo1            = $descr;
					$pdf1->descr1             = $numero;
					$pdf1->descr2             = db_numpre($k00_numpre, 0).'000';
					//.db_formatar($k00_numpar,'s',"0",3,"e");
					if (isset($obs)) {
					...
					if (!empty($HTTP_POST_VARS["ver_matric"])) {
						...
						$rsresultender   = pg_query($sqlEnder);
					...
					for ($x = 0; $x < $intnumrows; $x++) {
						...
						//   (float)substr($fc_calcula,14,13);
						//$valreceita;
						//$pdf1->arrayvalreceitas[$x]   = (substr($fc_calcula,12,14)+substr($fc_calcula,12,14)+substr($fc_calcula,12,14)-substr($fc_calcula,12,14))$valreceita;
						//   (float)substr($fc_calcula,14,13);
						//$valreceita;
					...
					$rsMsgCarneUnica    = pg_query($sqlMsgCarne);
					...
					if ($iTercDig == '7') {
						...
						$pdf1->descr15 = db_formatar($k00_valor, 'f');
						//($ninfla==''?'R$'.db_formatar($k00_valor,'f'):$ninfla.''.$k00_valor);
						$pdf1->valtotal = db_formatar($k00_valor, 'f');
						$pdf1->descr7   = db_formatar($k00_valor, 'f');
						//($ninfla==''?'R$'.db_formatar($k00_valor,'f'):$ninfla.''.$k00_valor);
						$pdf1->predescr7 = db_formatar($k00_valor, 'f');
       ...
       limit 1";
		...
		for ($x = 0; $x < $intnumrows; $x++) {
			...
			if ($recibopaga == false) {
				...
				$pdf1->arrayvalreceitas[$x] = $fc_calcula;
				//$valreceita;
			}
		...
		if (!empty($HTTP_POST_VARS["ver_matric"])) {
			...
			$rsInscr = pg_query($sSqlInscr) or die($sSqlInscr);
			...
			$rsNumCgm = pg_query($sSqlNumCgm) or die($sSqlNumCgm);
		...
		//Alterado devido msg de taxa de expediente
		$k00_msgparcvenc2 	=	"";
		$k00_msgparc2 		=	"";
		if ($pagabanco == 't') {
		...
		$resultVerParcel = pg_query($sqlVerParcel);
		...
		if ($linhasVerParcel > 0) {
			...
			if ($k00_dtvenc > $datahoje) {
				...
				$resultDesconto = pg_query($sqlDesconto);
