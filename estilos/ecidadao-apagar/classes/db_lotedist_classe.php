...
class cl_lotedist { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->j54_ponto = ($this->j54_ponto == ""?@$GLOBALS["HTTP_POST_VARS"]["j54_ponto"]:$this->j54_ponto);
     }else{
       $this->j54_idbql = ($this->j54_idbql == ""?@$GLOBALS["HTTP_POST_VARS"]["j54_idbql"]:$this->j54_idbql);
     }
   }
   ...
   function incluir ($j54_idbql){ 
     ...
     if($this->j54_ponto == null ){ 
       $this->erro_sql = " Campo Ponto nao Informado.";
       $this->erro_campo = "j54_ponto";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
     ...
     $result = @pg_query("insert into lotedist(
                                       j54_idbql 
                                      ,j54_codigo 
                                      ,j54_distan 
                                      ,j54_ponto 
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = " ($this->j54_idbql) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function alterar ($j54_idbql=null) { 
     ...
     if(isset($GLOBALS["HTTP_POST_VARS"]["j54_ponto"])){ 
       $sql  .= $virgula." j54_ponto = '$this->j54_ponto' ";
       $virgula = ",";
       if($this->j54_ponto == null ){ 
         $this->erro_sql = " Campo Ponto nao Informado.";
         $this->erro_campo = "j54_ponto";
     ...
     $sql .= " where  j54_idbql = $this->j54_idbql
";
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($j54_idbql=null) { 
