...
class cl_ativtipo { 
   ...
   function incluir ($q80_ativ,$q80_tipcal){ 
     ...
     $result = @pg_query("insert into ativtipo(
                                       q80_ativ 
                                      ,q80_tipcal 
                       )
                values (
   ...
   function alterar ($q80_ativ=null,$q80_tipcal=null) { 
     ...
     $sql .= " where  q80_ativ = $this->q80_ativ
 and  q80_tipcal = $this->q80_tipcal
";
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($q80_ativ=null,$q80_tipcal=null) { 
     $this->atualizacampos(true);
     $result = @pg_exec(" delete from ativtipo
                    where  q80_ativ = $this->q80_ativ
, q80_tipcal = $this->q80_tipcal
                    ");
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   // funcao do sql 
   function sql_query ( $q80_ativ=null,$q80_tipcal=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
     ...
     $sql .= "      inner join tipcalc  on  tipcalc.q81_codigo = ativtipo.q80_ativ";
     $sql .= "      inner join cadcalc  on  cadcalc.q85_codigo = tipcalc.q81_cadcalc";
     $sql .= "      inner join cadvencdesc  on  cadvencdesc.q92_codigo = tipcalc.q81_codven";
