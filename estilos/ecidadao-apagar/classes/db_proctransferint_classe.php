...
class cl_proctransferint { 
   ...
   function incluir ($p88_codigo){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = pg_query("insert into db_acount values($acount,1069,6522,'','".AddSlashes(pg_result($resaco,0,'p88_publico'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["p88_publico"]))
           $resac = pg_query("insert into db_acount values($acount,1069,6522,'".AddSlashes(pg_result($resaco,$conresaco,'p88_publico'))."','$this->p88_publico',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = @pg_exec($sql);
     if($result==false){ 
   ...
   function excluir ($p88_codigo=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
         $acount = pg_result($resac,0,0);
         $resac = pg_query("insert into db_acountkey values($acount,6498,'$this->p88_codigo','E')");
         ...
         $resac = pg_query("insert into db_acount values($acount,1069,6522,'','".AddSlashes(pg_result($resaco,$iresaco,'p88_publico'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
