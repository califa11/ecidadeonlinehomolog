...
class cl_termoini { 
   ...
   var $campos = "
                 parcel = int4 = codigo do parcelamento 
   ...
   function incluir ($parcel,$inicial){ 
     ...
     $result = @pg_query("insert into termoini(
                                       parcel 
                                      ,inicial 
                                      ,valor 
                                      ,juros 
   ...
   function alterar ($parcel=null,$inicial=null) { 
     ...
     $sql .= " where  parcel = $this->parcel
 and  inicial = $this->inicial
";
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($parcel=null,$inicial=null) { 
     $this->atualizacampos(true);
     $result = @pg_exec(" delete from termoini
                    where  parcel = $this->parcel
, inicial = $this->inicial
   ...
   function sql_query ( $parcel=null,$inicial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= "      inner join inicial on  inicial.inicial = termoini.inicial";
     $sql2 = "";
     if($dbwhere==""){
       if($parcel!=null ){
