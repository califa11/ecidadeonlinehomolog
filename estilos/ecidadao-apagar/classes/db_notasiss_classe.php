...
class cl_notasiss { 
   ...
   function incluir ($q09_nota){ 
      $this->atualizacampos();
     if($this->q09_descr == null ){ 
       $this->erro_sql = " Campo Descricao da nota fiscal nao Informado.";
       ...
       $this->q09_nota = $q09_nota; 
     if(($this->q09_nota == null) || ($this->q09_nota == "") ){ 
       $this->erro_sql = " Campo q09_nota nao declarado.";
     ...
     $result = @pg_query("insert into notasiss(
                                       q09_nota 
                                      ,q09_descr 
                       )
                values (
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Notas Fiscais ($this->q09_nota) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     $this->erro_sql = "Inclusao Efetivada com Sucesso\\n";
         $this->erro_sql .= "Valores : ".$this->q09_nota;
     $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
     $this->erro_status = "1";
     return true;
   ...
   function alterar ($q09_nota=null) { 
      $this->atualizacampos();
     $sql = " update notasiss set ";
     $virgula = "";
     if(isset($GLOBALS["HTTP_POST_VARS"]["q09_nota"])){ 
       $sql  .= $virgula." q09_nota = '$this->q09_nota' ";
       $virgula = ",";
       if($this->q09_nota == null ){ 
         $this->erro_sql = " Campo tipo de nota fiscal: nao Informado.";
     ...
     if(isset($GLOBALS["HTTP_POST_VARS"]["q09_descr"])){ 
     ...
     $sql .= " where  q09_nota = '$this->q09_nota'
";
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($q09_nota=null) { 
     $this->atualizacampos(true);
     $result = @pg_exec(" delete from notasiss
                    where  q09_nota = '$this->q09_nota'
                    ");
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   // funcao do sql 
   function sql_query ( $q09_nota=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
     ...
     if($dbwhere==""){
       if($q09_nota!=null ){
         $sql2 .= " where notasiss.q09_nota = '$q09_nota' "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
   ...
   // funcao do sql 
   function sql_query_file ( $q09_nota=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = split("#",$campos);
       $virgula = "";
