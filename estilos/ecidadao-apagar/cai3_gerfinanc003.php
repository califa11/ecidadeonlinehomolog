...
include_once ("fpdf151/impcarne.php");
include_once ("libs/db_barras.php");
...
if (!isset($emite_recibo_protocolo)) {
					...
					for ($contanumpres = 0; $contanumpres < sizeof($matnumpres); $contanumpres++) {
						...
						$sqlagrupa = "select distinct
                           ...
                           where k00_numpre = ".$resultado[0]."
                             ...
                             arrecad.k00_dtvenc between w17_dtini and w17_dtfim end  ";//and arrecad.k00_tipo = 137 ";
	...
	$Recibo = db_query($sql) or die($sql.pg_last_error());
} else {
...
if (isset($tipo_debito)) {
    ...
    k99_numpar";
		...
		$result = pg_query($sqlhist) or die($sqlhist);
...
//die(pg_result($Identificacao,0,"z01_bairro"));
//$z01_bairro;
...
if (isset($reemite_recibo)) {
	...
	//$sqlObs = "insert into recibopagahist values ($k03_numpre,'".$historico."')";
	//pg_query($sqlObs);
}
...
$pdf1->titulo8 = $tipoidentificacao;
// tipo de identificacao;
$pdf1->pretitulo8 = $tipoidentificacao;
// tipo de identificacao;
...
/***************************************************************************************************************************************/
$rsMsgcarne = pg_query("select k03_msgbanco from numpref where k03_anousu = ".db_getsession('DB_anousu'));
