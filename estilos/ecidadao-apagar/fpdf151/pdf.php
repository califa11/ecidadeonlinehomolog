...
//   echo ("select nomeinst,ender,munic,uf,telef,email,url,logo from db_config where codigo = ".db_getsession("DB_instit"));
   ...
   //$dados = pg_exec($conn,"select nomeinst, trim(ender)||','||trim(numero) as ender,munic,uf,telef,email,url,logo from db_config where codigo = ".db_getsession("DB_instit"));
    $url = @pg_result($dados,0,"url");
    $this->SetXY(1,1);
    //$this->Image('imagens/files/'.pg_result($dados,0,"logo"),7,3,20);
    $this->Image('imagens/files/e-cidade_logo_o.jpg',7,3,20);
