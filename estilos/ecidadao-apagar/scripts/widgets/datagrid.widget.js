...
function DBGrid(sName) {
  ...
  this.addRow =  function(aRow, lRender, lDisabled, lChecked) {
    ...
    if ((aRow instanceof Array)) {
      ...
      if (this.hasCheckbox) {
        ...
        var sCheckbox = "";
        sCheckbox    += "<input type='checkbox' id='chk"+aRow[this.checkBoxValue]+"' "+sDisabled+ " "+sChecked;
  ...
  this.show = function (oNode) {
    ...
    var sGrid = "<div class='container' id='grid"+this.sName+"'>"; 
    ...
    sGrid    += "<table class='table-header'  id='table"+this.sName+"header'>";
    ...
    sGrid    += "<div class='body-container' style='height:"+me.iHeight+"px;'>";
  ...
  this.renderRows = function (lReturnString, lResizeColumns) {
    ...
    var sRows = '';
    for (var iRowLength = 0; iRowLength < this.aRows.length; iRowLength++) {
       sRows += this.aRows[iRowLength].create();
    }
