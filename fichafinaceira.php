...
$sqlContraChequeLiberado = "select 1 from contrachequesliberados where ano = $iAnoUsu and mes = $iMesUsu and matricula = $iMatric";
$rsContraLiberado = db_query($sqlContraChequeLiberado);
 
/*  
if($sSigla  == 'r14' && strtotime("$iAnoUsu-$iMesUsu-01") > strtotime("2019-02-01") && pg_num_rows($rsContraLiberado) == 0 ){
  die(); 
}
*/
...
$sSqlDadosServidor .= "                                AND rh02_anousu = (case when rh02_instit = 1 then fc_anofolha(1) when rh02_instit = 3 then fc_anofolha(3) when rh02_instit = 4 then fc_anofolha(4) else fc_anofolha(1) end)
      ...
      <tr>  
        <td>
          <table width="100%">
			...
			<?if ( ($iMatric == 25042 && str_pad($iAnoUsu,2,'0',STR_PAD_LEFT) == '2017' && str_pad($iMesUsu,2,'0',STR_PAD_LEFT) == '01')  || ($iMatric == 26730 && str_pad($iAnoUsu,2,'0',STR_PAD_LEFT) == '2017' && str_pad($iMesUsu,2,'0',STR_PAD_LEFT) == '05')){?>
            <tr>
              <td class="labelForm">
               Situação:
              </td>
              <td class="dadosForm">
               <b> <font size=20 color=red>Cancelada</font></b>
              </td>              
            </tr>
			<?}?>
          </table>  
