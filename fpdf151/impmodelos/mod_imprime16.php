...
//		$this->objpdf->text($xcol+12,$xlin+11,$this->descr_lota);
		...
		if(strtoupper($this->municpref) == 'GUAIBA' || strtoupper($this->municpref) == 'ARAPIRACA'){
		  $this->objpdf->text($xcol+80,$xlin+117,'Margem Consignavel');
		}else{
		...
		for($ii = 0;$ii < $this->linhasenvelope ;$ii++) {
           	   ...
           	   if ( pg_result($this->recordenvelope,$ii,$this->tipo)  == 'P'){
	   	          ...
	   	          $this->objpdf->cell(93,3,pg_result($this->recordenvelope,$ii,$this->descr_rub),0,0,"L",0);
     		         ...
     		         /* ------------------------------------
						ALTERAÇÃO PARA EXIBIÇÃO DE TRIENIOS 
						--------------------------------------*/
						...
						if ($rubrica !== '0006') {
							...
							if (empty($this->f013)) {
								...
								$this->objpdf->cell(20,3, $this->f013,'f',0,0,"R",0);								
							}
                  ...
                  if($this->instit == 1 && strtoupper($this->municpref == 'GUAIBA') && ($rubrica == '0102' || $rubrica == '0109' || $rubrica == '0111' || $rubrica == '0195'  || $rubrica == '0196' || $rubrica == '0197' || $rubrica == '0198' )){
                    $margem_consignada += pg_result($this->recordenvelope,$ii,$this->valor);
                  }elseif($this->instit == 1 && strtoupper($this->municpref) == 'ARAPIRACA' && 
                 ...
                 $rubrica = trim(pg_result($this->recordenvelope,$ii,$this->rubrica));
                 if($this->instit == 1 && strtoupper($this->municpref) == 'ARAPIRACA' ){
		...
		$this->objpdf->text($xcol+5,$xlin+121,db_formatar($this->f010,'f'));
		$this->objpdf->text($xcol+40,$xlin+121,db_formatar($baseprev,'f'));
		if(strtoupper($this->municpref == 'GUAIBA') || strtoupper($this->municpref) == 'ARAPIRACA' ){
      if($this->instit == 1){
   		  $this->objpdf->text($xcol+75,$xlin+121,db_formatar((  (( $margem_consignada*30/100 ) - $margem_deduz ) < 0?0:(($margem_consignada*30/100 ) - $margem_deduz) )   ,'f'));
      }else{  
   		  $this->objpdf->text($xcol+75,$xlin+121,db_formatar($provento*30/100,'f'));
      }
		}else{
		...
		if($this->mes >= 11 and $this->ano >= 2019){
					//Alterado em 13/12/2019 em resposta ao Ofício nº PCTN-PGM 1175/2019 autorizado pelo Secretário Francis Henrique

					// Alterado em 29/12/2020 a pedido Gustavo Reis. Secretário-Adjunto ADM. Diego Bueno.
		$this->objpdf->multicell(125,4,'MENSAGEM: Os servidores que possuem empréstimo consignado descontado em folha de pagamento e o desconto não foi efetuado deverão procurar a instituição financeira.',0,"J");
		} else {
