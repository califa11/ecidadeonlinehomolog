...
if(isset($HTTP_POST_VARS["alt_senha"])) {
  ...
  $result = @pg_query("select senha from db_usuarios where senha = '$senha' ");
  if(@pg_num_rows($result) == 0 ) {
    msgbox("ERRO: Senha Inválida.");
   redireciona("digitafornecedor.php");
   exit;
  }
  $result = @pg_query("update db_usuarios set senha = '$senha_c1' where login = '$cgccpf'") or die(@pg_errormessage());
...
//MANDA UM E-MAIL DE CONFIRMAÇÃO
  ...
  $cgccpf = str_replace("/","",$cgccpf);
  $cgccpf = str_replace("-","",$cgccpf);
  $result = @pg_query("select z01_nome,z01_email,z01_cgccpf from cgm where trim(z01_cgccpf) = '$cgccpf'") or die(@pg_errormessage());
  if(@pg_num_rows($result) == 0) {
  ...
  $result = @pg_query("select login from db_usuarios where login = '$cgccpf'");
  //criptografia
...
";
  ...
  {
    $result = @pg_query("select senha from db_usuarios where login = '$cgccpf' and senha != '' ");
   ...
   {
      if( ( $DB_senha != md5(@pg_result($result,0,0)) )   )
...
//reenvio de senha
      ...
      if( ( $DB_senha != md5(@pg_result($result,1,0)) )   )
          
$result = @pg_query("select login,senha from db_usuarios where trim(login) = '$cgccpf' ");
  if(@pg_num_rows($result) == 0 ) {
...
<table width="657" height="100%" border="0" cellpadding="0" cellspacing="0" background="imagens/azul_ceu_O.jpg">
  ...
  <tr>
    <td height="173" valign="top">
        <?
         //pega número do cgm
         $result = @pg_query("SELECT z01_numcgm FROM cgm where Z01_CGCCPF='$cgccpf'");
         $dados = @pg_fetch_row( $result, 0 );
         $cgm = $dados[0];
         //realiza a pesquisa de empenhos
          $result = @pg_query("select Z01_CGCCPF, Z01_NUMCGM, Z01_NOME,
