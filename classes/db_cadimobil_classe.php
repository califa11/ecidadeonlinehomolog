...
class cl_cadimobil { 
   ...
   function incluir ($j63_numcgm){ 
     ...
     $result = @pg_query("insert into cadimobil(
                                       j63_numcgm 
                       )
                values (
                                $this->j63_numcgm 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = pg_query("insert into db_acountkey values($acount,2431,'$this->j63_numcgm','I')");
       $resac = pg_query("insert into db_acount values($acount,394,2431,'','".pg_result($resaco,0,'j63_numcgm')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($j63_numcgm=null) { 
     ...
     $virgula = "";
     if($this->j63_numcgm!="" || isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"])){ 
       $sql  .= $virgula." j63_numcgm = $this->j63_numcgm ";
       $virgula = ",";
       if($this->j63_numcgm == null ){ 
     ...
     $sql .= " where  j63_numcgm = $this->j63_numcgm
";
     $resaco = $this->sql_record($this->sql_query_file($this->j63_numcgm));
     if($this->numrows>0){       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,2431,'$this->j63_numcgm','A')");
       if(isset($GLOBALS["HTTP_POST_VARS"]["j63_numcgm"]))
         $resac = pg_query("insert into db_acount values($acount,394,2431,'".pg_result($resaco,0,'j63_numcgm')."','$this->j63_numcgm',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     $result = @pg_exec($sql);
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
   ...
   function excluir ($j63_numcgm=null) { 
     $this->atualizacampos(true);
     $resaco = $this->sql_record($this->sql_query_file($this->j63_numcgm));
     if(($resaco!=false)||($this->numrows!=0)){
       $resac = pg_query("select nextval('db_acount_id_acount_seq') as acount");
       $acount = pg_result($resac,0,0);
       $resac = pg_query("insert into db_acountkey values($acount,2431,'$this->j63_numcgm','E')");
       $resac = pg_query("insert into db_acount values($acount,394,2431,'','".pg_result($resaco,0,'j63_numcgm')."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     ...
     $sql2 = "";
      ...
      $sql2 .= " j63_numcgm = $this->j63_numcgm ";
}
