...
class cl_cgm { 
   ...
   function incluir ($z01_numcgm){ 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = pg_query("insert into db_acount values($acount,42,7309,'','".AddSlashes(pg_result($resaco,0,'z01_nomecomple'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       $resac = pg_query("insert into db_acount values($acount,42,7344,'','".AddSlashes(pg_result($resaco,0,'z01_dtvencimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["z01_nomecomple"]))
           $resac = pg_query("insert into db_acount values($acount,42,7309,'".AddSlashes(pg_result($resaco,$conresaco,'z01_nomecomple'))."','$this->z01_nomecomple',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         if(isset($GLOBALS["HTTP_POST_VARS"]["z01_dtvencimento"]))
           $resac = pg_query("insert into db_acount values($acount,42,7344,'".AddSlashes(pg_result($resaco,$conresaco,'z01_dtvencimento'))."','$this->z01_dtvencimento',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = @pg_exec($sql);
     if($result==false){ 
   ...
   function excluir ($z01_numcgm=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = pg_query("insert into db_acount values($acount,42,7309,'','".AddSlashes(pg_result($resaco,$iresaco,'z01_nomecomple'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         $resac = pg_query("insert into db_acount values($acount,42,7344,'','".AddSlashes(pg_result($resaco,$iresaco,'z01_dtvencimento'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $sql = " delete from cgm
                    where ";
