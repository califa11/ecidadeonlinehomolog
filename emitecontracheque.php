...
$sqlContraChequeLiberado = "select 1 from contrachequesliberados where ano = $anocalc and mes = $mescalc and matricula = $iMatric";

//die($sqlContraChequeLiberado);

$rsContraLiberado = db_query($sqlContraChequeLiberado);

if(false){ //($tipocalc  == 'r14') && (strtotime("$anocalc-$mescalc-01") > strtotime("2019-02-01")) && (pg_num_rows($rsContraLiberado) == 0 )){ 
   echo 'Contra Cheque Indisponível';
    db_redireciona('db_erros.php?fechar=true&db_erro=Contra Cheque Indisponível.');
   die();
}else
  { 
...
$filtro     = 'M';
//$msg        = 'Conforme parecer da Procuradoria e recomendação da Corregedoria, a Lei nº 1031/64 encontra-se revogada desde dezembro/1990, assim a partir de abril/2014 a gratificação de especialização será suprimida.';
$msg        = ''; 
$local      = '';
$tipo_local = 's';
$num_vias   = 1;
...
$pdf->SetFontSize(10);
/*
$pdf->Cell(0,4,'Está disponível no site da Prefeitura de Sete Lagoas -  www.setelagoas.mg.gov.br , acesso intranet, impressão',0,1,'C');
$pdf->Cell(0,4,'de contracheque, a atualização cadastral para preenchimento obrigatório. Para acesso deverá ser utilizado login e senha,  ',0,1,'C');
$pdf->Cell(0,4,'caso ainda não possua, deverá fazer a solicitação utilizando matrícula e CPF.',0,1,'C');
$pdf->Cell(0,4,'Lembramos que o prazo para cadastro será ate o dia 20 de maio de 2014.',0,1,'C');
*/
